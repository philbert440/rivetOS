// The `qwen-code` driver over fakes for the two things it wraps: the den term
// manager (PTY spawn/inject/Esc) and the qwen on-disk store. No `qwen` binary
// and no ~/.qwen required.
//
...
const SID = `qwen-code:${UUID}` as SessionId
...
/** Force the fallback sheet — tests do not depend on ~/.qwen/settings.json. */
const missingQwenFiles: SheetReaders = {
  readJson: (): never => {
...
const qwenEvent = (session: string, body: Record<string, unknown>): DenAgentEventLike =>
  ({
    v: 1,
    session,
    harness: 'qwen-code',
...
describe('capability flags are honest', () => {
  ...
  it('advertises -m and no effort flag when settings.json is missing', () => {
    const caps = makeDriver().driver.capabilities
    expect(caps.modelFlag).toBe('-m')
  ...
  it('rejects attachments even when the URI is a staged node-local path', async () => {
    ...
    await expectUnsupported(() =>
      driver.sendUserTurn(SID, {
        text: 'look',
        attachments: [{ mime: 'image/png', pathOrUri: '/home/example/.rivetos/den/uploads/x.png' }],
      }),
    )
  })
...
describe('identity + canonicalization', () => {
  it('mints `qwen-code:<uuid>` ids from qwen’s own ids', () => {
    expect(QwenCodeDriver.sessionId(UUID)).toBe(SID)
  ...
  it('lists store rows as canonical summaries', async () => {
    const { driver } = makeDriver({
      rows: [{ id: UUID, command: 'qwen', title: 'review the PR', updatedAt: 1_700_000_000_000 }],
    })
  ...
  it('ignores rows from another harness store in the same list', async () => {
    const { driver } = makeDriver({
      rows: [
        { id: UUID, command: 'qwen', title: 'mine', updatedAt: 2 },
...
describe('startSession', () => {
  it('pins the native id and spawns the `qwen` roster entry', async () => {
  ...
  it('rejects a non-uuid pin — `qwen --session-id` cannot honor it', async () => {
    const { driver } = makeDriver()
...
describe('resumeSession', () => {
  it('re-spawns with --resume, in a room named after the native id', async () => {
    const { driver, pty } = makeDriver({
      rows: [{ id: UUID, command: 'qwen', title: 't', updatedAt: 2 }],
    })
...
describe('sendUserTurn', () => {
  ...
  it('re-attaches (--resume) when the PTY was LRU-evicted between turns', async () => {
    ...
    expect(pty.spawns).toEqual([
      { key: 'qwen', session: UUID, resume: UUID },
      { key: 'qwen', session: UUID, resume: UUID },
    ])
  })
...
describe('subscribe maps den AgentEvents onto the contract', () => {
  it('streams paired tool calls; turn completion comes from the transcript', async () => {
    ...
    expect(seen.some((e) => e.type === 'tool-use' && e.name === 'read_file')).toBe(false)
  })

  it('ignores den rooms that are not qwen-code', () => {
  ...
  it('ignores the translator’s id-less fallback room (`unknown-<ppid>`)', () => {
    ...
    const seen: HarnessEvent[] = []
    driver.subscribeEvents((e) => seen.push(e))
    emitDen({ v: 1, session: 'unknown-4242', harness: 'qwen-code', type: 'session.start' })
    expect(seen).toEqual([])
  })

  it('adopts a qwen PTY spawned from the /term drawer (synthetic rivetos start)', () => {
    const { driver, emitDen } = makeDriver()
    const seen: HarnessEvent[] = []
    driver.subscribeEvents((e) => seen.push(e))
    emitDen({
  ...
  it('marks a session ended when its harness exits', async () => {
    ...
    const seen: HarnessEvent[] = []
    driver.subscribeEvents((e) => seen.push(e))
    emitDen(qwenEvent(UUID, { type: 'session.end' }))
...
describe('through the real registry', () => {
  ...
  it('registers under the qwen-code harness id and advertises its flags', () => {
    const { fakes, registry } = withRegistry()
    const [desc] = registry.list()
    expect(registry.list()).toHaveLength(1)
    expect(desc.harnessId).toBe('qwen-code')
  ...
  it('lists canonical ids, exactly once each', async () => {
    const { registry } = withRegistry([
      { id: UUID, command: 'qwen', title: 'a', updatedAt: 2 },
      { id: UUID2, command: 'qwen', title: 'b', updatedAt: 1 },
    ])
    const ids = (await registry.listSessions('qwen-code')).map((s) => s.sessionId)
    expect(ids).toEqual([SID, `qwen-code:${UUID2}`])
...
describe('transcript', () => {
  it('serves the hard-resync source for a canonical id', async () => {
    const { driver, store } = makeDriver({
      rows: [{ id: UUID, command: 'qwen', title: 't', updatedAt: 1 }],
    })
...
describe('close', () => {
  it('detaches the den tap', () => {
    const off = vi.fn()
    const driver = new QwenCodeDriver({ store: fakeStore().host(), events: () => off })
    driver.close()
