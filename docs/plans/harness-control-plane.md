...
- [x] **`reasoningText` accumulates unbounded in the hub live store** — capped (REASONING_TEXT_MAX 4096, sliding window in the shared nextReasoningText hook mirroring den's THOUGHT_MAX semantics; live tail only, committed thinking untouched) — neither `foldStream` nor `foldHarnessEvent` caps thinking accumulation, unlike the den reducer's `THOUGHT_MAX` sliding window (`packages/den-protocol/src/reducer.ts:92-95`). Pre-existing on the legacy path, but `reasoning-delta` now carries the claude-cli executor's real thinking token-streams, so a long turn grows the store without bound. Apply the same sliding-window cap in the shared `nextReasoningText` hook
