...
| grok-cli     | `plugins/providers/grok-cli/`     | Drives the Grok Build `grok` binary headlessly (`grok -p --output-format streaming-messages-json`); NDJSON deltas, subscription login, no API key                                                               |
...
| opencode-cli | `plugins/providers/opencode-cli/` | Drives the local OpenCode CLI (`opencode run --format json`); harness id `opencode`; default `model` `zai/glm-5.3-flash`. The CLI owns backend, endpoint, and credentials. Memory capture: `integrations/opencode/rivet-memory` (native plugin on `session.idle`) |
