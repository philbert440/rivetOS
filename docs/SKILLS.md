...
runtime:
  skill_dirs:
    ...
    - $RIVETOS_SHARED_DIR/skills    # Team-shared skills (optional)
```
...
The reference team-shared skill is `unslop` (`$RIVETOS_SHARED_DIR/skills/unslop/` when you publish it on the mesh): 31 rules for cutting AI tells from prose, ported from [pstack-claude](https://github.com/michael-denyer/pstack-claude) (MIT). Every agent applies it before publishing prose a human will read.
