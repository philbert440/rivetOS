...
import dev.rivet.ai.core.merge
import dev.rivet.ai.provider.CustomBody
import dev.rivet.ai.provider.CustomHeader
...
import dev.rivet.ai.ui.handleMessageChunk
import dev.rivet.ai.ui.limitContext
import dev.rivet.app.data.ai.transformers.InputMessageTransformer
import dev.rivet.app.data.ai.transformers.MessageTransformer
import dev.rivet.app.data.ai.transformers.OutputMessageTransformer
import dev.rivet.app.data.ai.transformers.onGenerationFinish
...
import dev.rivet.app.data.model.AssistantMemory
    ...
    private suspend fun generateInternal(
        ...
        if (stream) {
            ...
            providerImpl.streamText(
                ...
                it.agentModel?.let { agentModel ->
                    messages = messages.mapIndexed { index, message ->
                        if (index == messages.lastIndex) {
                            message.copy(agentModel = agentModel)
                        } else {
                            message
                        }
                    }
                }
                // Transient provider status pulse (e.g. "thinking · ~1.2k tok"); "" clears.
                it.status?.let { s -> processingStatus.value = s.ifBlank { null } }
