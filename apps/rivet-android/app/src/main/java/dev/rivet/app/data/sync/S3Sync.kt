package dev.rivet.app.data.sync
...
import dev.rivet.app.data.files.FileFolders
import dev.rivet.app.data.files.SkillPaths
import dev.rivet.app.data.datastore.Settings
import dev.rivet.app.data.datastore.SettingsStore
import dev.rivet.app.data.datastore.migration.SettingsJsonMigrator
import dev.rivet.app.data.sync.s3.S3Client
import dev.rivet.app.data.sync.s3.S3Config
import dev.rivet.app.utils.fileSizeToString
import java.io.File
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "rivethub_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "rivethub_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("rivethub_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
    ...
    suspend fun prepareBackupFile(config: S3Config): File = withContext(Dispatchers.IO) {
        ...
        ZipOutputStream(FileOutputStream(backupFile)).use { zipOut ->
            ...
            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                val dbFile = context.getDatabasePath("rivet_hub")
                if (dbFile.exists()) {
                    addFileToZip(zipOut, dbFile, "rivet_hub.db")
                }

                val walFile = File(dbFile.parentFile, "rivet_hub-wal")
                if (walFile.exists()) {
                    addFileToZip(zipOut, walFile, "rivet_hub-wal")
                }

                val shmFile = File(dbFile.parentFile, "rivet_hub-shm")
                if (shmFile.exists()) {
                    addFileToZip(zipOut, shmFile, "rivet_hub-shm")
                }
            }
            ...
            while (zipIn.nextEntry.also { entry = it } != null) {
                entry?.let { zipEntry ->
                    ...
                    when (zipEntry.name) {
                        ...
                        "rivet_hub.db", "rivet_hub-wal", "rivet_hub-shm" -> {
                            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                                val dbFile = when (zipEntry.name) {
                                    "rivet_hub.db" -> context.getDatabasePath("rivet_hub")
                                    "rivet_hub-wal" -> File(
                                        context.getDatabasePath("rivet_hub").parentFile,
                                        "rivet_hub-wal"
                                    )
