...
- Chat follows node via `NodeChatBackend`: local → `http://127.0.0.1:8765/v1` (bridge); remote unbound Rivet agent turns → gateway sessions `POST /api/sessions/:id` (conversation UUID = session id, title from first user message); remote harness-bound → control plane. Soft-sync falls back to gateway ring when no harness store row.
- `activeNodeDenUrl` and Rivet provider `baseUrl` always move together. Edit probe failure updates the roster only — never moves `activeNodeDenUrl` without a successful `switchNode` repoint.
- NodeSwitcher: Edit updates a roster row in place (position kept, active repoint). Cold start **once** migrates non-loopback `http://` roster rows to `https://` (`Settings.rosterHttpsMigrated` one-shot flag).
- **Id namespace invariant:** Room conversation UUID, harness native session id, and gateway session id share one string space. Routing order is: harness-bound (`harnessBinder.isBound`) → harness store row (`DenHarnessClient.tryList` hit) → gateway attach (`GET /api/sessions`). A harness-native id that equals a conversation UUID is claimed by the harness branch and never gateway-attaches. Do not create a gateway session under a harness native id. Cross-device resume of gateway-only sessions still requires the conversation already in Room (remote drawer lists harness/control-plane rows only).
