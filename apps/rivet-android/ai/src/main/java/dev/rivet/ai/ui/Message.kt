...
data class UIMessage(
    ...
    // The backing model that actually produced this message (e.g. "claude-fable-5"), for
    // CLI-bridge providers whose app-side "model" is just the agent slot (rivet-claude).
...
data class MessageChunk(
    ...
    val usage: TokenUsage? = null,
    val agentModel: String? = null,
    // Transient status pulse from the provider (e.g. "thinking · ~1.2k tok" for models
    // whose reasoning is encrypted). Empty string = clear. Display-only, never persisted.
