...
export function RoutineEditor({
  ...
  const cloudReady = Boolean(loadTeamAppSettings().desktopUrl.trim());
      ...
      <div className="max-h-[90vh] w-full max-w-[620px] overflow-y-auto rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline/40 bg-panel/95 px-5 py-4 backdrop-blur">
          <div>
            ...
            <div className="mt-0.5 text-[12px] text-ink-secondary">Give a persona scheduled work with a calendar you can trust.</div>
          </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Laptop size={15} />Persona node</div>
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Uses this persona's den and model.</div>
              </button>
              <button
                type="button"
              ...
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Cloud size={15} />Shared desktop</div>
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Runs on the household Ubuntu desktop every persona shares.</div>
              </button>
            </div>
            {runOn === "cloud" && (
              <div className={cn("mt-2 rounded-lg px-3 py-2 text-[11.5px] leading-relaxed", cloudReady ? "bg-accent/10 text-ink-secondary" : "border border-warning/25 bg-warning/10 text-warning")}>
                {cloudReady
                  ? "The shared desktop stays up for each run. Keep the household desktop reachable."
                  : "Shared desktop URL is set in Settings → Node."}
              </div>
            )}
          </div>
          <div>
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-medium text-ink-secondary">What should this persona do?</span>
        ...
        <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
          {routine && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{routine.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{routine.runOn === "cloud" ? "Shared desktop" : "Persona node"}</div></div>
          ...
          {run?.triggerSource === "webhook" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{run.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{run.runOn === "cloud" ? "Shared desktop" : "Persona node"}</div></div>
          ...
          {run?.status === "waiting" && <div className="rounded-xl border border-warning/30 bg-warning/10 px-3.5 py-3 text-[13px] text-warning">This persona needs your answer. Open its task to continue the run.</div>}
        </div>
          ...
          {routines.map((routine) => {
            ...
            return (
              <div key={routine.id} className="flex items-center gap-3 rounded-xl border border-hairline/40 bg-inset p-3">
                ...
                <div className="min-w-0 flex-1"><div className="truncate text-[14px] font-semibold text-ink">{routine.name}</div><div className="mt-0.5 truncate text-[11.5px] text-ink-secondary">{bot?.name ?? "Deleted persona"} · {scheduleLabel(routine)}</div></div>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-ink-secondary">{section === "calendar" ? "Run persona tasks on a schedule." : "Run persona tasks when an event arrives."}</p>
          </div>
      ...
      {section === "webhooks" ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-8">
          <div className="max-w-[430px] text-center">
            ...
            <h2 className="text-[18px] font-semibold text-ink">Put the household on a rhythm</h2>
