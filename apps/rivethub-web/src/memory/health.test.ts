...
const now = Date.now()
const healthy: MemoryHealthResponse = {
  status: 'ok',
  embeddings: { status: 'ok' },
  embedQueueDepth: 50,
  observedAt: new Date(now).toISOString(),
  queueStatus: 'available',
  capture: { status: 'unknown', impact: 'Not measured' },
}
...
it('makes stale, failed and partial observations visible', () => {
  ...
  expect(
    memoryHealthState(
      { ...healthy, capture: { status: 'unknown', impact: 'Not measured' } },
      false,
      now,
    ).label,
  ).toContain('partial')
  expect(memoryHealthState({ ...healthy, status: 'degraded' }, false, now).label).toContain(
    'attention',
  )
})

it('keeps unmeasured capture and restricted queues neutral', () => {
  expect(memoryHealthState(healthy, false, now).tone).toBe('good')
  expect(memoryHealthState({ ...healthy, queueStatus: 'restricted' }, false, now).tone).toBe('good')
})
