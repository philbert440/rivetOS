...
const pumpStore: OutboundPumpStore = {
  ...
  requeue: (sid, id, note) => useChat.getState().requeueOutbound(sid, id, note),
  fail: (sid, id, note) => useChat.getState().failOutbound(sid, id, note),
  restoreFailed: (sid, item, note) => useChat.getState().restoreOutboundFailed(sid, item, note),
