...
- `docs/ROADMAP.md`.
- Provider `@rivetos/provider-llama-server` — consolidated into `@rivetos/provider-openai-compat`, which now covers vLLM, TGI, llama.cpp's `llama-server`, Groq, Together, Fireworks, LocalAI, and any OpenAI-compatible server. Migration: change `provider: llama-server` → `provider: openai-compat` in your config; `base_url` and `model` carry over unchanged. The boot validator emits a hard error pointing at the replacement when the old key is still present.
