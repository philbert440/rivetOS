...
- When a herdr-backed coding agent exits, den reaps the PTY and mux session after a short grace so chat inject 409s instead of writing into the leftover shell (#791). Adopted harness panes stay closed to inject until a `pane list`/`agent list` probe (or a working|idle|blocked frame) proves a live agent; `POST /term` during the grace mints a new pty immediately.
