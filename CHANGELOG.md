...
- Harness capability `launchModel`: a harness that applies its `modelFlag` (`--model`/`-m`) at spawn declares it explicitly, via one `LAUNCH_MODEL_HARNESSES` set in `sheetForHarness` — claude-code, grok-build, kimi-code, opencode, pi, qwen-code. Excludes codex (per-turn `turnOptions` path) and hermes (no model choice). Distinct from `turnOptions`; the spawn model id is still validated against the harness sheet by `POST /term { model }` (#814).
