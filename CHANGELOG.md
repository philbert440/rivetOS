...
- Pre-spawn model picker for conversations whose resolved harness declares `launchModel` (claude-code today): shown only before the conversation is bound to a live harness session, lists only that harness's own sheet models (never a catalog default or another harness's), rides `POST /term { model }`, and is fixed for the conversation's life once it spawns. Cleared when the thread's agent/harness changes or when the settled sheet no longer offers the stored model (#814).
