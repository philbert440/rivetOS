...
- den serves presets from the DataHub (`ros_agent_presets` on `config.pgUrl` / `RIVETOS_PG_URL`) and falls back to the per-node `agents.json` when Postgres is absent or the table is not ready yet. The first time the table answers ready, that file is imported once and renamed aside; imported rows get a directory on disk. A name that already exists (case-insensitively) is imported as `"<name> (<node>)"`, then `"<name> (<node> 2)"`, instead of being dropped. Preset names are unique case-insensitively on every den, including file-only dens.
- Presets carry `node`, `directory`, and `sharedLink`. The hosting den materializes the directory (mode 0700) and a `rivet-shared` symlink to the shared directory. `sharedLink: false` does not create that symlink; turning it back on links it again. The directory is not deleted with the preset.
