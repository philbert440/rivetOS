...
- Chat: fix a first message that stuck at "sending" forever. When the control plane adopts a fresh draft mid-send (bare uuid → canonical `<harness>:<uuid>`), the store rekey moved the in-flight `sending` outbound item to the new key, but the pump that fired it dequeues under the retired key and the new key's pump refuses to touch a `sending` item — orphaning it. Rekey now drops the in-flight `sending` item (its inject already reached the resolvable session; the committed turn supersedes the optimistic bubble) while still moving `queued` items so the new pump sends them.
