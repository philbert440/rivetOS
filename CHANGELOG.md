...
- Detect harness exit under herdr: when a coding agent's process exits and herdr drops the pane to a shell, the den now marks the PTY harness-dead (debounced against flicker). `ptyForSession` hides it and `inject` refuses, so `POST /term/inject` returns `409` and the client respawns (`--resume`) instead of silently injecting chat turns into a leftover bash prompt (#791).
