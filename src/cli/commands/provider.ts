...
export default async function provider(providerName: string): Promise<void> {
  ...
  switch (providerName) {
    ...
    case 'xai':
      ...
      break;
    case 'google':
      await handleSimpleProvider('google', 'https://generativelanguage.googleapis.com/v1beta', 'GOOGLE_API_KEY', action);
      break;
    case 'ollama':
...
async function handleAnthropic(action: string): Promise<void> {
  switch (action) {
    ...
    case 'status': {
      ...
      console.log(`  Expires: ${new Date(tokens.expiresAt).toLocaleString()} ${expired ? '(EXPIRED)' : '(valid)'}`);
      console.log(`  Refresh token: ${hasRefresh ? '✅ available (auto-refresh enabled)' : '❌ none'}`);
    ...
    case 'status': {
      ...
      try {
        ...
        if (name === 'xai') {
          ...
          const res = await fetch(`${baseUrl}/models?key=${key}`, { signal: AbortSignal.timeout(5000) });
          console.log(`  Connectivity: ${res.ok ? '✅' : '❌'} (${res.status})`);
        }
      } catch (err: any) {
