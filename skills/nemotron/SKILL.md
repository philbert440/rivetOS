...
Local NVIDIA AI models running on GERTY (pve3, 192.168.1.50:9400). Zero API cost, zero latency to cloud.
...
```bash
curl -s -F "file=@/path/to/audio.ogg" http://192.168.1.50:9400/transcribe
```
...
```bash
curl -s -X POST http://192.168.1.50:9400/rerank \
...
### Health Check
```bash
curl -s http://192.168.1.50:9400/health
```
