 ...
 *   delegation:before — before delegating to another agent
 *   delegation:after  — after delegation completes
 */
...
export interface DelegationAfterContext extends HookContextBase {
  ...
  usage?: { promptTokens: number; completionTokens: number }
  /** Names of tools the delegate called */
  toolsUsed?: string[]
...
export interface TurnReflectContext extends HookContextBase {
  ...
  /** Token usage */
  usage?: { promptTokens: number; completionTokens: number }
  /** Why this turn triggered reflection */
  complexity: {
  ...
  source: 'complexity' | 'periodic' | 'manual'
  /** User message that started the turn */
  userMessage: string
}

/** Context for skill:before — fires before a skill is loaded */
export interface SkillBeforeContext extends HookContextBase {
  event: 'skill:before'
  /** Skill name */
  skillName: string
  /** Skill location (file path) */
  skillLocation: string
  /** Triggers that matched */
  matchedTriggers: string[]
  /** Match score from SkillManager */
  matchScore: number
  /** If set to true, skip loading this skill */
  skip?: boolean
  /** Reason for skipping */
  skipReason?: string
}

/** Context for skill:after — fires after a skill has been used */
export interface SkillAfterContext extends HookContextBase {
  event: 'skill:after'
  /** Skill name */
  skillName: string
  /** Whether the skill execution succeeded (user accepted the result) */
  success: boolean
  /** Tools used during the skill-guided turn */
  toolsUsed: string[]
  /** Number of tool iterations */
  iterations: number
  /** Duration of the turn in ms */
  durationMs: number
  /** Token usage during skill execution */
  usage?: { promptTokens: number; completionTokens: number }
}
