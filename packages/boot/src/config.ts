...
export interface WorkflowsSection {
  ...
  enabled?: boolean
  /**
   * Workflow ids agents may start via the `workflow_start` tool.
   * Empty / omitted = fail closed (agents start nothing).
...
export interface TasksSection {
   ...
   * id (`claude-code | grok-build | kimi-code | hermes | deepseek-harness | codex | pi`).
   *
