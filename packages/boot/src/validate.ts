...
const KNOWN_PROVIDERS: Record<string, Set<string>> = {
  ...
  'openai-compat': new Set(['model', 'base_url', 'api_key', 'max_tokens', 'temperature', 'top_p', 'repeat_penalty', 'name']),
  'llama-server': new Set(['model', 'base_url', 'api_key', 'max_tokens', 'temperature', 'top_p', 'repeat_penalty', 'name']),
};
...
const KNOWN_CHANNELS: Record<string, Set<string>> = {
  ...
  discord: new Set(['bot_token', 'owner_id', 'allowed_guilds', 'allowed_channels', 'allowed_users', 'channel_bindings', 'mention_only']),
  voice: new Set(['bot_token', 'xai_api_key', 'guild_id', 'allowed_users', 'voice', 'instructions', 'transcript_dir']),
  'voice-discord': new Set(['bot_token', 'xai_api_key', 'guild_id', 'allowed_users', 'voice', 'instructions', 'transcript_dir']),
};
...
export function validateConfig(config: unknown): ValidationResult {
  ...
  // === runtime (required) ===
  if (!cfg.runtime) {
    issues.push({ severity: 'error', path: 'runtime', message: 'Missing required section "runtime"' });
  ...
  // === agents (required) ===
  if (!cfg.agents) {
    issues.push({ severity: 'error', path: 'agents', message: 'Missing required section "agents" — define at least one agent' });
  } else if (typeof cfg.agents !== 'object' || Array.isArray(cfg.agents)) {
    issues.push({ severity: 'error', path: 'agents', message: '"agents" must be an object mapping agent names to their config' });
  } else {
  ...
  // === providers (required) ===
  if (!cfg.providers) {
    issues.push({ severity: 'error', path: 'providers', message: 'Missing required section "providers" — define at least one provider' });
  } else if (typeof cfg.providers !== 'object' || Array.isArray(cfg.providers)) {
    issues.push({ severity: 'error', path: 'providers', message: '"providers" must be an object mapping provider names to their config' });
  } else {
    ...
    if (!Array.isArray(runtime.skill_dirs)) {
      ...
      for (let i = 0; i < runtime.skill_dirs.length; i++) {
        if (typeof runtime.skill_dirs[i] !== 'string') {
          issues.push({ severity: 'error', path: `runtime.skill_dirs[${i}]`, message: 'Each skill_dirs entry must be a string path' });
        }
      }
    }
  }
...
function validateHeartbeat(hb: unknown, index: number, issues: ValidationIssue[]): void {
  ...
  if (!entry.agent || typeof entry.agent !== 'string') {
    issues.push({ severity: 'error', path: `${path}.agent`, message: 'Heartbeat requires a string "agent" field' });
  }

  if (!entry.schedule) {
    issues.push({ severity: 'error', path: `${path}.schedule`, message: 'Heartbeat requires a "schedule" field (e.g., "30m", "1h")' });
  }
  ...
  if (entry.quiet_hours !== undefined) {
    if (typeof entry.quiet_hours !== 'object' || Array.isArray(entry.quiet_hours)) {
      ...
      if (typeof qh.start !== 'number' || qh.start < 0 || qh.start > 23) {
        issues.push({ severity: 'error', path: `${path}.quiet_hours.start`, message: '"quiet_hours.start" must be a number 0-23' });
      }
      if (typeof qh.end !== 'number' || qh.end < 0 || qh.end > 23) {
        issues.push({ severity: 'error', path: `${path}.quiet_hours.end`, message: '"quiet_hours.end" must be a number 0-23' });
      }
    }
  }
}

function validateCodingPipeline(pipeline: Record<string, unknown>, issues: ValidationIssue[]): void {
  for (const key of Object.keys(pipeline)) {
    if (!KNOWN_PIPELINE_KEYS.has(key)) {
      issues.push({ severity: 'warning', path: `runtime.coding_pipeline.${key}`, message: `Unknown coding_pipeline key "${key}"` });
    }
  }

  if (pipeline.max_build_loops !== undefined) {
    if (typeof pipeline.max_build_loops !== 'number' || pipeline.max_build_loops < 1) {
      issues.push({ severity: 'error', path: 'runtime.coding_pipeline.max_build_loops', message: '"max_build_loops" must be a positive integer' });
    }
  }

  if (pipeline.max_validation_loops !== undefined) {
    if (typeof pipeline.max_validation_loops !== 'number' || pipeline.max_validation_loops < 1) {
      issues.push({ severity: 'error', path: 'runtime.coding_pipeline.max_validation_loops', message: '"max_validation_loops" must be a positive integer' });
    }
  }

  if (pipeline.auto_commit !== undefined && typeof pipeline.auto_commit !== 'boolean') {
    issues.push({ severity: 'error', path: 'runtime.coding_pipeline.auto_commit', message: '"auto_commit" must be a boolean' });
  }
}

function validateAgents(agents: Record<string, unknown>, fullConfig: Record<string, unknown>, issues: ValidationIssue[]): void {
  if (Object.keys(agents).length === 0) {
    issues.push({ severity: 'error', path: 'agents', message: '"agents" is empty — define at least one agent' });
    return;
  }
...
function validateProviders(providers: Record<string, unknown>, issues: ValidationIssue[]): void {
  if (Object.keys(providers).length === 0) {
    issues.push({ severity: 'error', path: 'providers', message: '"providers" is empty — define at least one provider' });
    return;
  }
    ...
    if (!KNOWN_PROVIDERS[name]) {
      ...
      for (const key of Object.keys(provider)) {
        if (!knownKeys.has(key)) {
          issues.push({ severity: 'warning', path: `${path}.${key}`, message: `Unknown key "${key}" for provider type "${name}"` });
        }
      }
    }
  ...
  for (const [name, channelCfg] of Object.entries(channels)) {
    ...
    if (!KNOWN_CHANNELS[name]) {
      ...
      for (const key of Object.keys(channel)) {
        if (!knownKeys.has(key)) {
          issues.push({ severity: 'warning', path: `${path}.${key}`, message: `Unknown key "${key}" for channel type "${name}"` });
        }
      }
    }
...
function validateMemory(memory: Record<string, unknown>, issues: ValidationIssue[]): void {
  ...
  if (memory.postgres) {
    if (typeof memory.postgres !== 'object' || Array.isArray(memory.postgres)) {
      issues.push({ severity: 'error', path: 'memory.postgres', message: '"memory.postgres" must be an object' });
    } else {
      const pg = memory.postgres as Record<string, unknown>;
      for (const key of Object.keys(pg)) {
        if (!KNOWN_MEMORY_POSTGRES_KEYS.has(key)) {
          issues.push({ severity: 'warning', path: `memory.postgres.${key}`, message: `Unknown memory.postgres key "${key}"` });
        }
      }
    }
  }
...
export function formatValidationResult(result: ValidationResult): string {
  ...
  if (result.valid && result.warnings.length === 0) {
    ...
    lines.push('');
    lines.push(`✅ Config is valid (${result.warnings.length} warning${result.warnings.length === 1 ? '' : 's'}).`);
  } else {
    lines.push('');
    lines.push(`❌ Config has ${result.errors.length} error${result.errors.length === 1 ? '' : 's'}${result.warnings.length > 0 ? ` and ${result.warnings.length} warning${result.warnings.length === 1 ? '' : 's'}` : ''}.`);
  }
