...
describe('acquireEmbeddedPg (real PGlite)', () => {
  it('boots, migrates, searches, resets session state, multiplexes, and attaches', async () => {
    delete process.env.RIVETOS_EMBED_URL
    ...
    try {
      ...
      const sqlFiles = readdirSync(findMigrationsDir()).filter((f) => f.endsWith('.sql')).length
      expect(sqlFiles).toBeGreaterThan(0)
      expect(applied.rows[0].n).toBe(sqlFiles)
      ...
      await A.end()
      // The reset runs on the handler's close event, fire-and-forget: poll until it lands.
      {
        const deadline = Date.now() + 5000
        for (;;) {
          const probeB = new Client({ connectionString: pgUrl })
          await probeB.connect()
          const cur = (await probeB.query<{ application_name: string }>('SHOW application_name'))
            .rows[0].application_name
          await probeB.end()
          if (cur !== 'client-A' || Date.now() > deadline) break
          await sleep(50)
        }
      }
      ...
      await count.connect()
      const n = await count.query<{ n: number }>('select count(*)::int as n from ros_messages')
      await count.end()
      expect(n.rows[0].n).toBe(21)

      const attached = await acquireEmbeddedPg(resolved, { log })
      const otherPort = await freePort()
      const attachedOther = await acquireEmbeddedPg(
        {
          ...resolved,
          port: otherPort,
          pgUrl: `postgres://postgres:postgres@127.0.0.1:${String(otherPort)}/postgres`,
        },
        { log },
      )
      expect(attachedOther.owned).toBe(false)
      expect(attachedOther.pgUrl).toBe(pgUrl) // the owner's port from the lock, not the config's
...
describe('acquireEmbeddedPg lock edges', () => {
  it('refuses to attach when the lock holder is alive but its port is dead', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'rivet-pglite-lock-'))
    try {
      const port = await freePort()
      writeFileSync(
        join(dir, 'rivetos-owner.lock'),
      ...
      const resolved = {
        ...
        autoMigrate: false,
        maxConnections: 4,
        pgUrl: `postgres://postgres:postgres@127.0.0.1:${String(port)}/postgres`,
        liteMode: true,
      }
      await expect(acquireEmbeddedPg(resolved, { log, attachTimeoutMs: 800 })).rejects.toThrow(
        /nothing listens/,
      )
    } finally {
  ...
  it('treats a lock with a dead pid as stale and takes ownership', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'rivet-pglite-stale-'))
    let handle: EmbeddedPgHandle | undefined
    try {
      const port = await freePort()
      // pid 2^22-1 is above the default Linux pid_max; no such process.
      writeFileSync(join(dir, 'rivetos-owner.lock'), JSON.stringify({ pid: 4194303, port: 1 }))
      const resolved = {
        dataDir: dir,
        port,
        autoMigrate: false,
      ...
      handle = await acquireEmbeddedPg(resolved, { log })
      expect(handle.owned).toBe(true)
      const lock = JSON.parse(readFileSync(join(dir, 'rivetos-owner.lock'), 'utf8')) as {
        pid: number
        port: number
      }
