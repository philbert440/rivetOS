...
const KNOWN_TOP_LEVEL_KEYS = new Set(['runtime', 'agents', 'providers', 'channels', 'memory'])
...
/** Known agent keys */
const KNOWN_AGENT_KEYS = new Set(['provider', 'default_thinking'])
  ...
  for (const [name, providerCfg] of Object.entries(providers)) {
    ...
    if (!KNOWN_PROVIDERS[name]) {
      issues.push({
        severity: 'warning',
        path,
        message: `Unknown provider type "${name}" — make sure boot.ts has a handler for it`,
      })
    } else {
  ...
  for (const [name, channelCfg] of Object.entries(channels)) {
    ...
    if (!KNOWN_CHANNELS[name]) {
      issues.push({
        severity: 'warning',
        path,
        message: `Unknown channel type "${name}" — make sure boot.ts has a handler for it`,
      })
    } else {
