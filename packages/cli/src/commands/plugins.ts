 ...
 */
async function checkProviderAuth(name: string, config: Record<string, unknown>): Promise<{ ok: boolean; detail: string }> {
...
function checkChannelAuth(name: string, config: Record<string, unknown>): { ok: boolean; detail: string } {
...
function checkMemoryAuth(name: string, config: Record<string, unknown>): { ok: boolean; detail: string } {
  ...
  switch (subcommand) {
    case 'list': {
      ...
      for (const name of installedProviders) {
        if (!providers[name]) {
          results.push({ type: 'provider', name, status: 'available', detail: 'installed but not in config' });
        }
      }
      ...
      for (const name of installedChannels) {
        if (!channels[name]) {
          results.push({ type: 'channel', name, status: 'available', detail: 'installed but not in config' });
        }
      }
      ...
      for (const name of installedMemory) {
        if (!memory[name]) {
          results.push({ type: 'memory', name, status: 'available', detail: 'installed but not in config' });
        }
      }
