      ...
      try {
        ...
        if (parsed.triggers) {
          if (typeof parsed.triggers === 'string') {
            result.triggers = parsed.triggers.split(',').map((t: string) => t.trim().toLowerCase()).filter(Boolean);
          } else if (Array.isArray(parsed.triggers)) {
            result.triggers = parsed.triggers.map((t: unknown) => String(t).trim().toLowerCase()).filter(Boolean);
          }
        }
        return result;
      } catch {
      ...
      for (const line of fmBlock.split('\n')) {
        ...
        switch (key) {
          ...
          case 'description':
            ...
            break;
          case 'triggers':
            result.triggers = value.split(',').map((t) => t.trim().toLowerCase()).filter(Boolean);
            break;
        }
      }
      return result;
...
function countTriggersFromDescription(description: string): number {
  const stopWords = new Set([
    ...
    'use', 'when', 'what', 'how', 'where', 'which', 'who', 'whom',
    'why', 'if', 'then', 'else', 'also', 'up', 'down',
  ]);
