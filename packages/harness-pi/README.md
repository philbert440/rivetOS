...
(`@earendil-works/pi-coding-agent`), headlessly.

```
pi --print --mode json [--session <id>] [--model <model>] [--thinking <effort>] <prompt>
```

<!-- REVIEWER-CONFIRM: bin name (`pi` vs `pi-coding-agent`) and the exact print/JSON flag set. -->
...
2. **Stream.** Print/JSON is treated as NDJSON on stdout — assistant text,
   `tool_start` / `tool_end`, a `session` (or terminal `result`) carrying the
   native session id — translated to den `message.agent` / `tool.start` /
   `tool.end`. The native id is canonicalized to `pi:<native>`.
3. **Reconcile.** If the stream carried no usage, after the child exits the
   executor reads the session's own `transcript.jsonl` and reports the turn's
   tokens from there. Post-hoc, because the process has exited: no tailing, no
   attribution race, no torn-read handling beyond skipping the one line a
   SIGKILL can damage.
...
- **No per-turn MCP injection.** Servers are assumed to come from pi's own
...
tasks:
  harnesses:
    pi:
      ...
      model: glm-4.5             # optional; --model
      effort: medium             # optional; --thinking
      cwd: /srv/rivetos/work     # default: the workspace dir
      home: ~/.pi/agent          # optional PI_HOME override
```
...
Recommended default backend: z.ai GLM, reusing the coding-plan key already on
this fleet. Not pinned here.

## Testing

`npm test` runs the shared executor-conformance suite plus pi specifics
against a FAKE `pi` binary writing a pi-shaped transcript into a throwaway
`PI_HOME`. No provider tokens are spent and the operator's `~/.pi` is never
touched.
