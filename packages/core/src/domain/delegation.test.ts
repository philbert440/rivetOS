...
function createMockHookPipeline(): HookPipeline & { hooks: HookRegistration[] } {
  ...
  return {
    hooks,
    register: (hook: HookRegistration) => {
      hooks.push(hook)
    },
...
describe('DelegationEngine', () => {
  describe('basic delegation', () => {
    ...
    it('passes per-call model override to provider, falling back to agent default', async () => {
      ...
      const router = {
        getAgents: () => [
          { id: 'grok', name: 'grok', provider: 'xai', model: 'grok-configured-default' },
        ],
  ...
  describe('noDelegation flag', () => {
    it('does not give delegate_task tool when noDelegation is true', async () => {
      ...
      const result = await engine.delegate(createRequest({ noDelegation: true }), 0)
      expect(result.status).toBe('completed')
    })
  ...
  describe('createDelegationTool', () => {
    ...
    it('tool execute delegates and returns response', async () => {
      ...
      const result = await tool.execute({ to_agent: 'grok', task: 'test task' }, undefined, {
        agentId: 'opus',
      })
    ...
    it('tool returns status prefix on failure', async () => {
      ...
      const result = await tool.execute({ to_agent: 'nonexistent', task: 'test' }, undefined, {
        agentId: 'opus',
      })
    ...
    it('uses provided chain depth', async () => {
      ...
      const result = await tool.execute({ to_agent: 'grok', task: 'test' }, undefined, {
        agentId: 'opus',
      })
...
describe('filterToolsForAgent', () => {
  ...
  it('returns all tools when agent has no filter entry', () => {
    expect(filterToolsForAgent(allTools, 'grok', { local: { exclude: ['shell'] } })).toEqual(
      allTools,
    )
  })
