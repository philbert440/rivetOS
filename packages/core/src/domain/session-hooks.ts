 ...
 */
export function createSessionStartHook(ctx: SessionHooksContext): HookRegistration<SessionStartContext> {
  return {
 ...
 */
export function createSessionSummaryHook(ctx: SessionHooksContext): HookRegistration<SessionEndContext> {
  return {
    ...
    handler: async (hookCtx) => {
      ...
      if (hookCtx.totalTokens) {
        ...
        lines.push(`- Tokens: ${total.toLocaleString()} (${hookCtx.totalTokens.prompt.toLocaleString()} in / ${hookCtx.totalTokens.completion.toLocaleString()} out)`);
      }
 ...
 * workspace changes.
 */
export function createAutoCommitHook(ctx: SessionHooksContext): HookRegistration<SessionEndContext> {
  return {
 ...
 * survived compaction.
 */
export function createPostCompactHook(ctx: SessionHooksContext): HookRegistration<CompactAfterContext> {
  return {
