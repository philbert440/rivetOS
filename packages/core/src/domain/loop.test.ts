...
describe('AgentLoop', () => {
  ...
  it('should inject steer message into conversation', async () => {
    ...
    const provider = makeMockProvider({
      ...
      onCall: ({ prompt }) => {
        ...
        for (const m of prompt) {
          if (m.role === 'system') {
            recorded.push({ role: 'system', content: typeof m.content === 'string' ? m.content : '' });
          } else if (m.role === 'user') {
    ...
    const steerMsg = secondCallMessages.find((m) =>
      typeof m.content === 'string' && m.content.includes('STEER'),
    );
    assert.ok(steerMsg, 'Steer message should be injected');
    assert.ok(
      typeof steerMsg.content === 'string' && steerMsg.content.includes('Actually, use the -la flag'),
    );
  });
