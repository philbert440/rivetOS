 ...
 *     (`@rivetos/harness-opencode`), `pi` has a real executor over the `pi`
 *     binary (`@rivetos/harness-pi`), and `qwen-code` has a real executor over
 *     headless `qwen -p` (`@rivetos/harness-qwen-code`). Each registers a
 *     rejection only where boot's binary probe fails, carrying the probe's
 *     own reason.
 ...
 * `kimi-code`, `opencode`, `pi` and `qwen-code` all have one, so a rejection
 * registered for any of them can only come from boot's probe (binary not
 * resolvable, package would not load) and carries that reason instead.
 */
