# PI.md — RivetOS Memory

_Distilled guidance for pi CLI sessions using the `rivet-memory` plugin._

Install by copying this file (or its memory section) into your project rules or
global pi configuration so the discipline is always active. The installer copies
this content to `~/.pi/agent/AGENTS.md`.
...
1. Discover the exact qualified tool names for the RivetOS MCP server
   (e.g. `rivetos__memory_browse` or `memory_browse` depending on how pi
   surfaces MCP tools).
...
is just as valid as one from a previous pi session. Only filter by `agent` when the
...
workspace. The underlying model (pi / DeepSeek) is an implementation detail.
