...
`rivet-deepseek`, etc.) via the RivetOS MCP server.
...
## pi CLI Specific Tool Usage

1. Discover the exact MCP tool names for the RivetOS server. Depending on how
   pi surfaces MCP tools, they may appear as `memory_search` /
...
`"rivet-grok"`, `"rivet-kimi"`, `"rivet-gpt"`, `"rivet-deepseek"`, etc. This is a
