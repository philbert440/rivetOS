# OPENCODE.md — RivetOS Memory

_Distilled guidance for OpenCode CLI sessions using the `rivet-memory` plugin._

Install by copying this file (or its memory section) into your project rules or
global OpenCode configuration so the discipline is always active. If OpenCode
respects the vendor-neutral `AGENTS.md` convention, the installer copies this
content to `~/.config/opencode/AGENTS.md`.
...
3. **"No results" is not the end.** Treat it as a signal to change strategy:
   ...
   - Check other agents (`agent != "rivet-glm"`)
...
1. Discover the exact qualified tool names for the RivetOS MCP server
   (e.g. `rivetos__memory_browse` or `memory_browse` depending on how OpenCode
   surfaces MCP tools).
...
is just as valid as one from a previous OpenCode session. Only filter by `agent` when the
...
workspace. The underlying model (OpenCode / GLM) is an implementation detail.
