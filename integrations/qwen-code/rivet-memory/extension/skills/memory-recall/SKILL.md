...
(`rivet-claude`, `rivet-hermes`, `rivet-grok`, `rivet-kimi`, `rivet-gpt`,
`rivet-deepseek`, `rivet-qwen`, etc.) via the RivetOS MCP server.
...
## Qwen Code Specific Tool Usage

1. Discover the exact MCP tool names for the RivetOS server. Depending on how
   Qwen Code surfaces MCP tools, they may appear as `memory_search` /
...
`"rivet-grok"`, `"rivet-kimi"`, `"rivet-gpt"`, `"rivet-deepseek"`, `"rivet-qwen"`,
etc. This is a feature. A question like "what did we decide about X" should
surface the best answer regardless of which Rivet face originally did the work.
...
- Qwen Code capture is a native extension (`UserPromptSubmit` / `Stop` / `SessionEnd` hooks) that tails gemini-style session jsonl. Pre-compaction context is whatever the session file still holds.
