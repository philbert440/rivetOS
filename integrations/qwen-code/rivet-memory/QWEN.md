# QWEN.md — RivetOS Memory

_Distilled guidance for Qwen Code sessions using the `rivet-memory` plugin._

Install by copying this file (or its memory section) into your project rules or
global Qwen Code configuration so the discipline is always active. The installer
ships this as a qwen extension skill (`/memory-recall`).
...
3. **"No results" is not the end.** Treat it as a signal to change strategy:
   ...
   - Check other agents (`agent != "rivet-qwen"`)
...
1. Discover the exact qualified tool names for the RivetOS MCP server
   (e.g. `rivetos__memory_browse` or `memory_browse` depending on how Qwen Code
   surfaces MCP tools).
...
Memory is shared. A fact discovered by `rivet-claude`, `rivet-hermes`, or `rivet-kimi`
is just as valid as one from a previous Qwen Code session. Only filter by `agent` when
the user specifically wants lineage-limited results.
...
workspace. The underlying model (Qwen Code) is an implementation detail.
