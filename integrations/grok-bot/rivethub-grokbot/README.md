...
MCP launchers that call `rivetos_load_env` **parse** `KEY=VALUE` lines (optional `export` prefix, quotes, last-wins). They never `source` or evaluate the file, so `$(…)` and backticks do not run. Other harness hook and capture scripts (Claude Code, Grok Build, Codex, …) still `source` `~/.rivetos/.env`. Unquoted `$VAR`, `${VAR}`, `${VAR:-default}`, `${VAR-default}`, and a leading `~` or `~/…` expand. The same `$VAR` / `${VAR}` / `${VAR:-default}` / `${VAR-default}` forms expand inside double quotes, matching bash — `RIVETOS_ROOT="$HOME/rivetos"` becomes `$HOME/rivetos`. A double-quoted `\$` is a literal `$` (what persist writes). Tilde does not expand inside quotes (`"~/x"` stays `~/x`); `a~b` stays literal. Unquoted `#` starts a comment after whitespace (`KEY= #c` and `KEY=a #b` drop the rest; `KEY=#x` and `KEY=a#b` keep the hash) or at line start. Double-quoted escapes match bash: only `\\`, `\"`, `\$`, and `` \` `` lose their backslash (`\n`, `\t`, `\p` stay two characters). Any other shell form the parser cannot reproduce (other `${…}` operators, `'a'\''b'`, a trailing-backslash continuation, unescaped `$(…)` / backticks) is kept literal and prints one warning to stderr that names the key, never the value.
