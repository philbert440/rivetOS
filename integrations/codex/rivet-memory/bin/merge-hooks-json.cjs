...
function applyMerge(dest, fragmentPath, pluginPath) {
  ...
  for (const [event, groups] of Object.entries(fragment.hooks || {})) {
    ...
    // Replace any existing Rivet entries with the freshly built command (repairs
    // an old unquoted registration or a moved install root); foreign inner
    // commands and group metadata are preserved.
