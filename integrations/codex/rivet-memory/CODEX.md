# CODEX.md — RivetOS Memory

_Distilled guidance for Codex CLI sessions using the `rivet-memory` plugin._

Install by copying this file (or its memory section) into your project rules or
global Codex configuration so the discipline is always active. If Codex respects
the vendor-neutral `AGENTS.md` convention, the installer copies this content to
`~/.codex/AGENTS.md`.
...
3. **"No results" is not the end.** Treat it as a signal to change strategy:
   ...
   - Check other agents (`agent != "rivet-gpt"`)
...
is just as valid as one from a previous Codex session. Only filter by `agent` when the
