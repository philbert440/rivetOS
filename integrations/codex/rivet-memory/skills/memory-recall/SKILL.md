...
(`rivet-claude`, `rivet-hermes`, `rivet-grok`, `rivet-kimi`, `rivet-gpt`, etc.) via the RivetOS MCP server.
...
## Codex CLI Specific Tool Usage

1. Discover the exact MCP tool names for the RivetOS server. Depending on how
   Codex surfaces MCP tools, they may appear as `memory_search` /
...
`"rivet-grok"`, `"rivet-kimi"`, `"rivet-gpt"`, etc. This is a feature. A question
like "what did we decide about X" should surface the best answer regardless of
which Rivet face originally did the work.
