 ...
 * 3. Live RivetOS MCP HTTP mount: call memory_search and format it as
 *    agent context (the recall pipe T3 cannot inject automatically).
 */
...
check('built-in MCP is type http /mcp', T3_SURFACE.builtInMcp.type === 'http' && T3_SURFACE.builtInMcp.path === '/mcp')
check(
...
const shipped = JSON.parse(fs.readFileSync(path.join(ROOT, 't3-plugin.json'), 'utf8')) as {
  name: string
  mcpUrl: string
}
check('shipped t3-plugin.json name is rivetos-memory', shipped.name === 'rivetos-memory')
check('shipped t3-plugin.json has /mcp url', shipped.mcpUrl.endsWith('/mcp'))

const mcpHttp = JSON.parse(fs.readFileSync(path.join(ROOT, 'mcp-http.json'), 'utf8')) as {
  mcpServers: { rivetos: { type: string; url: string; headers?: unknown } }
}
...
const pluginMeta = JSON.parse(fs.readFileSync(path.join(ROOT, 'plugin.json'), 'utf8')) as {
  t3code: { firstClassPluginApi: boolean; contextInjectionHooks: boolean }
}
...
check(
  'empty recall is honest',
  formatMemoriesAsContext('  ', 'x').includes('No memories matched'),
)
...
check('store intent calls memory_append', stored.tool === 'memory_append' && String(stored.text).includes('note'))
const summarized = await invokeIntent(fakeCall, 'summarize', {})
check('summarize intent returns mapping note', summarized.tool === null && summarized.text === SUMMARIZE_NOTE)
