...
| `memory_browse` | Chronological message browsing. For reviewing sessions and catching up on activity. Prefer `window=today\|yesterday\|this_morning\|this_week\|last_24h` for time-bounded questions (local-TZ midnight → UTC; Hermes parity). Spaced forms like `this morning` are normalized; unknown values hard-fail with the valid list (no silent unfiltered results). |
