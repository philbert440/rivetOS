...
export function createMemoryTools(
  ...
  const tools: Tool[] = [
    createSearchTool(searchEngine, expander, config),
  ];
...
function createSearchTool(
  ...
  return {
    ...
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search query — natural language question or keywords' },
        mode: {
          type: 'string',
          enum: ['fts', 'trigram', 'regex'],
          description: 'Search mode: fts (full-text, default), trigram (fuzzy/typo-tolerant), regex (pattern match)',
        },
        scope: {
          type: 'string',
        ...
        synthesize: {
          type: 'boolean',
          description: 'Use LLM to synthesize a focused answer from results (default: false). Requires Rivet Local.',
        },
      },
      required: ['query'],
    },
    async execute(args: Record<string, unknown>): Promise<string> {
      ...
      sections.push(`Found ${results.length} results (${summaryHits.length} summaries, ${messageHits.length} messages)\n`);
      ...
      if (expandedSummaries.length > 0) {
        ...
        for (const { hit, children, sourceMessages } of expandedSummaries) {
          ...
          const period = hit.earliestAt && hit.latestAt
            ? `${fmtDate(hit.earliestAt)} → ${fmtDate(hit.latestAt)}`
            : `${fmtDate(hit.createdAt)}`;

          sections.push(`**[${hit.kind ?? 'summary'}]** (${age}d ago, score: ${hit.score.toFixed(3)}, period: ${period})`);
          ...
          if (sourceMessages.length > 0) {
            ...
            for (const msg of sourceMessages.slice(0, 8)) {
              const msgContent = msg.content.length > 300
        ...
        if (remaining.length > 0) {
          ...
          for (const hit of remaining) {
            const age = Math.floor((Date.now() - hit.createdAt.getTime()) / MS_PER_DAY);
            const preview = hit.content.length > 300
              ? hit.content.slice(0, 300) + '…'
              : hit.content;
            sections.push(`- [${hit.kind ?? 'summary'}] (${age}d ago, score: ${hit.score.toFixed(3)}) ${preview}`);
          }
          sections.push('');
        }
        ...
        for (const hit of summaryHits) {
          ...
          const preview = hit.content.length > 300
            ? hit.content.slice(0, 300) + '…'
            : hit.content;
          sections.push(`- [${hit.kind ?? 'summary'}/${hit.id}] (${age}d ago, score: ${hit.score.toFixed(3)}) ${preview}`);
        }
        sections.push('');
      }
      ...
      if (messageHits.length > 0) {
        ...
        for (const hit of messageHits) {
          const age = Math.floor((Date.now() - hit.createdAt.getTime()) / MS_PER_DAY);
          const preview = hit.content.length > 400
            ? hit.content.slice(0, 400) + '…'
            : hit.content;
          sections.push(`- [${hit.agent}/${hit.role}] (${age}d ago, score: ${hit.score.toFixed(3)}) ${preview}`);
        }
      }
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
      ...
      try {
        ...
        if (byAgent.rows.length > 0) {
          sections.push(
            '\n**By agent:**\n' +
            byAgent.rows.map((r: any) => `  ${r.agent}: ${Number(r.count).toLocaleString()}`).join('\n'),
          );
        }
        ...
        if (byRole.rows.length > 0) {
          sections.push(
            '\n**By role:**\n' +
            byRole.rows.map((r: any) => `  ${r.role}: ${Number(r.count).toLocaleString()}`).join('\n'),
          );
        }
        ...
        if (byKind.rows.length > 0) {
          const totalSummaries = byKind.rows.reduce((sum: number, r: any) => sum + Number(r.count), 0);
          sections.push(
            `\n**Summaries:** ${totalSummaries.toLocaleString()} total\n` +
            byKind.rows.map((r: any) => `  ${r.kind}: ${Number(r.count).toLocaleString()} (max depth: ${r.max_depth})`).join('\n'),
          );
        } else {
        ...
        const queueStatus = queueTotal === 0 ? '✅ caught up' :
                           queueTotal < 50 ? `⏳ ${queueTotal} pending` :
        ...
        const msgPct = Number(me.total) > 0 ? ((Number(me.embedded) / Number(me.total)) * 100).toFixed(1) : '0';
        const sumPct = Number(se.total) > 0 ? ((Number(se.embedded) / Number(se.total)) * 100).toFixed(1) : '0';
        sections.push(
          `\n**Embedding coverage:**` +
        ...
        const unsumStatus = unsumCount < 50 ? '✅' :
                           unsumCount < 200 ? '⏳' : '⚠️';
        sections.push(
        ...
        if (needsCompaction.rows.length > 0) {
          sections.push(
            ...
            needsCompaction.rows.map((r: any) =>
