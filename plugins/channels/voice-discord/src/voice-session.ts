...
export class VoiceSession {
  ...
  constructor(connection: VoiceConnection, config: VoicePluginConfig) {
    ...
    if (config.postgresConnectionString) {
      ...
      this.postgresPool =
        config.postgresPool ??
        new Pool({ connectionString: config.postgresConnectionString, max: 2 })
    }
    ...
    if (name === 'search_memories') {
      if (!this.postgresPool) {
        return JSON.stringify({
          error: 'Memory search not configured (no postgresConnectionString)',
        })
      }
      try {
  ...
  setVoice(voice: string): void {
    this.xai.updateSession(
      voice,
      this.config.instructions ?? "You are Rivet, Phil's AI assistant. Keep responses concise.",
    )
  }
