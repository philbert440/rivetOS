...
describe('search_grep', () => {
  beforeAll(() => {
    ...
    writeFileSync(
      join(TEST_DIR, 'node_modules', 'dep', 'index.js'),
  ...
  it('returns no-match message', async () => {
    const tool = createSearchGrepTool()
    const result = await tool.execute({ pattern: 'zzzznotfound' }, undefined, {
      workingDir: TEST_DIR,
    })
  ...
  it('supports case-insensitive search', async () => {
    const tool = createSearchGrepTool()
    const result = await tool.execute({ pattern: 'HELLO', case_insensitive: true }, undefined, {
      workingDir: TEST_DIR,
    })
  ...
  it('supports fixed string search', async () => {
    const tool = createSearchGrepTool()
    const result = await tool.execute({ pattern: 'foo, bar', fixed_strings: true }, undefined, {
      workingDir: TEST_DIR,
    })
  ...
  it('supports include filter', async () => {
    const tool = createSearchGrepTool()
    const result = await tool.execute({ pattern: 'hello', include: '*.ts' }, undefined, {
      workingDir: TEST_DIR,
    })
  ...
  it('searches a specific file', async () => {
    const tool = createSearchGrepTool()
    const result = await tool.execute({ pattern: 'Hello', path: join(TEST_DIR, 'README.md') })
    assert.ok(result.includes('Hello'))
  })
