 ...
 * Uses Node 24's built-in fs/promises.glob.
 */
...
const DEFAULT_EXCLUDE_DIRS = new Set(['node_modules', '.git', 'dist', 'build', '.next', 'coverage'])
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        cwd: {
          type: 'string',
          description: 'Directory to search from (optional, defaults to working directory)',
        },
      },
      required: ['pattern'],
    },
