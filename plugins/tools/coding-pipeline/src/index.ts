...
export function createPlugin(config?: CodingPipelineConfig): ToolPlugin {
  ...
  return {
    name: '@rivetos/tool-coding-pipeline',
    version: '0.1.0',
    description: 'Autonomous coding pipeline — Grok builds, self-reviews, Opus validates',
    async init(_config: PluginConfig) {},
    getTools() {
      return [createCodingPipelineTool(pipeline)]
    },
    async shutdown() {},
    /** Expose the pipeline instance for boot to wire up late-bound executors */
    get pipeline() {
      return pipeline
    },
