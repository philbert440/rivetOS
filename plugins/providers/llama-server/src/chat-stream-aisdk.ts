/**
 * AI SDK-backed implementation of llama.cpp llama-server's OpenAI-compatible `chatStream`.
 *
 ...
 * - **vLLM sampling extensions** — `top_k` and `min_p` (llama.cpp sampling) are sent via
 ...
 *   maps from the server's `reasoning_content` field when llama.cpp is started
 *   with `--reasoning-format deepseek`. The legacy inline
 *   `<think>...</think>` content fallback was dropped — operators should
 *   configure `--reasoning-format deepseek` on the server instead.
...
export interface LlamaServerAiSdkContext {
...
export async function* chatStreamAiSdk(
  ...
  // Inject llama.cpp sampling extensions via transformRequestBody. Standard sampling fields
